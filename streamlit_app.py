...
# ⚠️ 請在此處替換為您在「第一步」獲得的全新網址 (確認不含 /u/0/)
...
def get_data_via_gas(target_date):
    ...
    try:
        ...
        with requests.Session() as session:
            # allow_redirects=True 確保能跟隨 Google 的身分跳轉
            res = session.get(api_url, headers=headers, timeout=30, allow_redirects=True)
            
            # 診斷：若拿到 HTML 代表 GAS 部署權限仍被阻擋
            if res.text.strip().startswith("<!DOCTYPE html>"):
                return None, "拿到 Google 登入網頁，請重新執行 GAS『新增部署』並設為所有人存取。"
...
st.caption("透過 Google Cloud 代理請求，解決證交所封鎖雲端 IP 的問題。")
